        ...
        if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $this->arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"]))
        {
            ...
            if(check_bitrix_sessid())
            {
                if(empty($this->arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $this->arParams["REQUIRED_FIELDS"]))
                {
                    ...
                    if((empty($this->arParams["REQUIRED_FIELDS"]) || in_array("TIME", $this->arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_time"]) <= 1)
                        $this->arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_TIME");
                ...
                $this->arResult["AUTHOR_TIME"] = htmlspecialcharsbx($_POST["user_time"]);
            }
            else
